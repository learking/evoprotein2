...
public class PathTreeLikelihood extends Distribution {
    ...
    public double calculateLogP() throws Exception{
    	...
    	if(firstTimeCalculation){
        	...
        	double oneSiteP = calculatePathTreeLogP();
        	logP = oneSiteP;
        	return logP;
    	}
    }
    
    public double calculatePathTreeLogP() throws Exception{	
    	...
    	for (int i=0 ; i< m_pathTree.get().getNodeCount(); i++){
				...
				for (int seqSite = 0; seqSite < endSeq.getSequence().length; seqSite++) {
					...
					double currentBranchLength = m_pathTree.get()
							.getNode(endNodeNr).getLength();
					List<SubstitutionEvent> currentSubstitutionEvents = currentBranch
					...
					if (currentSubstitutionEvents.size() == 0) {
						...
						if (beginNucleotide != endNucleotide) {
							...
							// throw new
							// Exception("begin and end nucleotide should be the same when there is no substitution along this branch!");
						}

						// diagonal elements are already negative
						transitionOutEventCode = beginNucleotide * 4
								+ beginNucleotide;
						...
						for (int substitutionIndex = 0; substitutionIndex < currentSubstitutionEvents
							...
							int previousNucleotide = currentSubstitutionEvents
									.get(substitutionIndex)
									.getPreviousNucleotide();
							int currentNucleotide = currentSubstitutionEvents
									.get(substitutionIndex)
									.getCurrentNucleotide();
							// determine the last nucleotide
							if (substitutionIndex == (currentSubstitutionEvents
									.size() - 1)) {
								lastNucleotide = currentNucleotide;
							}

							double currentTimeInterval = currentSubstitutionEvents
									.get(substitutionIndex).getTimeInterval();
							...
							transitionEventCode = previousNucleotide * 4
									+ currentNucleotide;
							oneSiteLogP += Math
									.log(instantMatrix[transitionEventCode])
									+ instantMatrix[transitionOutEventCode]
									* currentTimeInterval;
						}

						double unchangedHeight = currentBranchLength
								- cumulativeHeight;
						// sanity check

						if (lastNucleotide != endNucleotide) {
							System.out.println("first:" + beginNucleotide
									+ " end:" + endNucleotide + " last:"
									+ lastNucleotide + " size:"
									+ currentSubstitutionEvents.size());
							System.out.println("last!=end");
							throw new Exception(
									"after last substituion, the nucleotide should be the same as the end of the branch! ");
						}

						transitionOutEventCode = lastNucleotide * 4
								+ lastNucleotide;
						oneSiteLogP += instantMatrix[transitionOutEventCode]
								* unchangedHeight;
					}
    ...
    // one site version of the PathTree likelihood calculation
